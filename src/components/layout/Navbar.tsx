...
import { Menu, X, Search, Crown, UserCircle, ExternalLink, FileText, BookOpen, File } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-surface/80 backdrop-blur-md border-b border-outline-variant shadow-sm transition-colors">
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-surface-container border border-outline-variant rounded-lg shadow-xl max-h-[500px] overflow-y-auto z-50">
                {searchResults.length > 0 ? (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <button
                      onClick={handleBrowserSearch}
                      className="px-4 py-2 border border-outline-variant rounded-md text-sm text-on-surface flex items-center justify-center gap-2 hover:bg-surface-container-high transition-colors w-full"
                    >
                      <ExternalLink className="w-4 h-4" />
                      Search in browser
                    </button>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="absolute top-16 left-0 w-full bg-surface border-b border-outline-variant shadow-lg md:hidden z-40 animate-fade-in-up">
          <div className="p-4 flex flex-col gap-4">
            ...
            {/* Mobile Links */}
            <nav className="flex flex-col border-t border-outline-variant pt-2 mt-2">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
            ...
            <div className="flex flex-col gap-3 mt-2 border-t border-outline-variant pt-4">
              <Link to="/JMCPlus" onClick={() => setMobileMenuOpen(false)} className="bg-secondary text-on-secondary py-2 rounded-lg text-center font-label-caps text-label-caps flex items-center justify-center gap-2">
                <Crown className="w-4 h-4" />
                JMC Plus
              </Link>
              {session && (
                <Link to="/dashboard" onClick={() => setMobileMenuOpen(false)} className="border border-outline-variant py-2 rounded-lg text-center font-label-caps text-label-caps text-on-surface">
                  Student Dashboard
                </Link>
              )}
              {session && role === 'admin' && (
                <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="border border-outline-variant py-2 rounded-lg text-center font-label-caps text-label-caps text-on-surface">
                  Admin Panel
                </Link>
              )}
            </div>
